  ...
  useEffect(() => {
    const checkAvailability = async () => {
      ...
      try {
          ...
          // 2. Time Check (20h30 - 23h50 GMT)
          const now = new Date();
          const utcMinutes = now.getUTCHours() * 60 + now.getUTCMinutes();
          const openMinutes = 20 * 60 + 30;
          const closeMinutes = 23 * 60 + 50;
          const isOpenTime = utcMinutes >= openMinutes && utcMinutes <= closeMinutes;

          if (!isOpenTime) {
            setIsQuizAvailable(false);
            setAvailabilityMessage("Le quiz est ouvert uniquement entre 20H30 et 23H50 (GMT).");
            return;
          }
      ...
      default:
        return (
          <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in duration-500">
            {/* Hero Section */}
            <div className="bg-white/90 backdrop-blur rounded-2xl shadow-2xl overflow-hidden border border-emerald-100">
              <div className="p-8 text-center">
                 {/* Decorative Icon - Logo */}
                 <div className="mx-auto w-28 h-28 bg-white rounded-full flex items-center justify-center mb-6 shadow-lg overflow-hidden border border-emerald-100">
                    <img 
                        src="logo.png" 
                        alt="Logo ASAA" 
                ...
                <h2 className="text-3xl display-font font-bold text-gray-800 mb-4">Bienvenue, {user.username}</h2>
                ...
                {isQuizAvailable ? (
                  <button 
                    ...
                    className="w-full sm:w-auto bg-gradient-to-r from-emerald-700 to-emerald-900 hover:from-emerald-800 hover:to-emerald-950 text-white font-bold py-4 px-10 rounded-full transition transform hover:scale-105 shadow-lg flex items-center justify-center gap-2 mx-auto"
                  >
                    <span>Commencer le Quiz du Jour</span>
                    <CheckCircle2 size={20} />
                  </button>
                ) : (
                  <div className="bg-amber-50/90 border border-amber-200 rounded-xl p-6 max-w-md mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               <div className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-950 p-6 rounded-xl shadow-lg text-white flex flex-col justify-center">
